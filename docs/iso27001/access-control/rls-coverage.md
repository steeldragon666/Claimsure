...
| `signup_decision`          | Forensic signup-approval ledger (0088); the row predates the tenant on approve and has no tenant on deny, so tenant-scoping cannot apply               | Written only via `privilegedSql` by the signup pipeline; `REVOKE ALL ON cpa_app` so a future shared-grant migration cannot expose it; never read by app code                                              |
| `auth_magic_link`          | Passwordless-login tokens (0091) looked up by `token_hash` before any session/tenant context exists — the token IS the auth signal                     | Token hash is the secret; `consumed_at` atomic update prevents replay; `privilegedSql`-only, `REVOKE ALL ON cpa_app` — same pattern as `magic_link_token`                                                 |
| `auth_login_attempt`       | Per-IP login rate-limit ledger (0092); records every attempt by `client_ip` before the user lookup, so there is no tenant scope                        | Written only by the pre-session login route via `privilegedSql`; `REVOKE ALL ON cpa_app`; rows are count-only within a 1-hour window and carry no tenant data                                             |
...
| Drift fix       | 2026-05-29 | Aaron    | Migrations `0088_signup_decision.sql`, `0091_auth_magic_link.sql`, `0092_auth_login_attempt.sql` shipped three intentionally non-RLS auth/signup-infra tables (all written via `privilegedSql` with `REVOKE ALL ON cpa_app`, all pre-tenant) without updating the exempt list; CI audit caught it. Added entries with rationale and compensating controls. |
