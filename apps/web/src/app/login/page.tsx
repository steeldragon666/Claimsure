...
type SubmitState = 'idle' | 'submitting' | 'no_workspace' | 'error';
...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // 200 → session cookie set; navigate to the redirect the API chose.
      if (res.status === 200) {
        const body = (await res.json().catch(() => ({}))) as {
          ok?: boolean;
          redirect?: string;
        };
        router.push(body.redirect ?? '/consultant');
        return;
      }

      // 404 → no approved workspace for this email. Friendly message +
      // signup link (rendered in the no_workspace state below).
      if (res.status === 404) {
        setState('no_workspace');
        return;
      }

      // 422 / 429 / 5xx → surface the API message if provided.
      const body = (await res.json().catch(() => ({}))) as { message?: string };
      const message = body.message ?? 'Could not log in. Please try again.';
        ...
        <div className="grid flex-1 items-center gap-10 py-12 lg:grid-cols-[0.95fr_1.05fr]">
          <section>
            ...
            <p className="mt-6 max-w-xl font-body text-base leading-8 text-[#cdc7bd]">
              Sign in to your approved firm workspace to pick up evidence capture, narrative
              drafting, and claim-pack review where you left off.
            </p>
            <div className="mt-10 divide-y divide-[#f0ebe2]/10 border-y border-[#f0ebe2]/10">
              {[
                'Enter the work email tied to your approved workspace.',
                'No password to remember — access is tied to your email.',
                'Need a workspace instead? Signup approval is instant.',
              ].map((item, index) => (
          ...
          <section className="border border-[#f0ebe2]/20 bg-[#131316]/95 p-6 shadow-[0_30px_90px_rgba(0,0,0,0.55)]">
            {state === 'no_workspace' ? (
              <div className="space-y-6 py-6">
                ...
                <div>
                  <h2 className="font-display text-3xl font-light">No workspace found.</h2>
                  <p className="mt-4 font-body text-sm leading-7 text-[#cdc7bd]">
                    No approved workspace found for this email. Sign up to create one.
                  </p>
                </div>
              ...
              <form onSubmit={(e) => void onSubmit(e)} className="space-y-5">
                <div>
                  ...
                  <h2 className="mt-3 font-display text-3xl font-light">Log in to ArchiveOne</h2>
                  <p className="mt-3 font-body text-sm leading-7 text-[#cdc7bd]">
                    Enter the work email tied to your approved ArchiveOne account.
                  </p>
                </div>
                ...
                <p className="border-t border-[#f0ebe2]/10 pt-5 font-mono text-[11px] uppercase tracking-[0.06em] text-[#8a857c]">
                  Need a workspace?{' '}
                  <Link
                    href="/signup"
                    className="text-[#e1a23a] underline decoration-[#e1a23a]/60 underline-offset-4 hover:decoration-[#e1a23a]"
                  >
                    Sign up
                  </Link>
                </p>
