...
export async function listTimeEntries(
  ...
  const body = await apiFetch<ListTimeEntriesResponse>(
    `/v1/time-entries?${qs.toString()}`,
  );
  return body.time_entries;
}

export async function setApportionment(
  id: string,
  apportionment_pct: number,
): Promise<TimeEntry> {
  const body = await apiFetch<{ time_entry: TimeEntry }>(
